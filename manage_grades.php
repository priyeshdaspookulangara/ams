...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['save_grade'])) {
        ...
        if (empty($grade_name)) {
            ...
            // Check for duplicate grade name (unless updating the same record)
            $check_sql = "SELECT id FROM grades WHERE grade_name = '$grade_name'" . ($grade_id > 0 ? " AND id != $grade_id" : "");
            $check_res = mysqli_query($conn, $check_sql);

            if (mysqli_num_rows($check_res) > 0) {
                $message = "Grade name '$grade_name' already exists.";
                $message_type = 'error';
            } else {
                if ($grade_id > 0) { // Update
                    ...
                    if (mysqli_query($conn, $sql)) {
                        ...
                        $message = "Error updating grade: " . mysqli_error($conn);
                        $message_type = 'error';
                    }
                } else { // Create
                    $sql = "INSERT INTO grades (grade_name) VALUES ('$grade_name')";
                    if (mysqli_query($conn, $sql)) {
                        $message = "Grade '$grade_name' created successfully.";
                        $message_type = 'success';
                    } else {
                        $message = "Error creating grade: " . mysqli_error($conn);
                        $message_type = 'error';
                    }
                }
            }
        }
    } elseif (isset($_POST['delete_grade'])) { // Handle Delete Grade
        $grade_id = (int)$_POST['grade_id'];
        // Check if grade is used in class_sections before deleting
        ...
        if (mysqli_num_rows($usage_res) > 0) {
            $message = "Cannot delete grade. It is currently assigned to one or more class sections. Please remove assignments first.";
            $message_type = 'error';
        } else {
            ...
            if (mysqli_query($conn, $sql)) {
                ...
                $message = "Error deleting grade: " . mysqli_error($conn);
                $message_type = 'error';
            }
        }
    }
...
// For editing, fetch grade details if an edit_id is provided
$edit_grade = null;
if (isset($_GET['edit_id'])) {
    ...
    if ($edit_res && mysqli_num_rows($edit_res) > 0) {
        $edit_grade = mysqli_fetch_assoc($edit_res);
    } else {
        $message = "Grade not found for editing.";
        $message_type = 'error';
    }
}
...
<body>
    ...
    <div class="container">
        ...
        <div class="form-section">
            <h2><?php echo $edit_grade ? 'Edit Grade' : 'Create New Grade'; ?></h2>
            <form action="manage_grades.php<?php echo $edit_grade ? '?edit_id=' . $edit_grade['id'] : ''; ?>" method="POST">
                ...
                <?php endif; ?>
                <label for="grade_name">Grade Name (e.g., "Grade 1", "Class X"):</label>
                <input type="text" name="grade_name" id="grade_name" value="<?php echo $edit_grade ? htmlspecialchars($edit_grade['grade_name']) : ''; ?>" required>
                <input type="submit" name="save_grade" value="<?php echo $edit_grade ? 'Update Grade' : 'Create Grade'; ?>">
                <?php if ($edit_grade): ?>
                    <a href="manage_grades.php" style="margin-left:10px; text-decoration:none; padding:10px 15px; background-color:#6c757d; color:white; border-radius:4px;">Cancel Edit</a>
                <?php endif; ?>
            </form>
        </div>
                    ...
                    <?php while ($grade = mysqli_fetch_assoc($grades_result)): ?>
                        <tr>
                            ...
                            <td class="action-links">
                                <a href="manage_grades.php?edit_id=<?php echo $grade['id']; ?>" class="button">Edit</a>
                                <form action="manage_grades.php" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this grade? This cannot be undone if it is not in use.');">
                                    <input type="hidden" name="grade_id" value="<?php echo $grade['id']; ?>">
                                    <button type="submit" name="delete_grade" class="delete">Delete</button>
                                </form>
                            </td>
                        </tr>
                    <?php endwhile; ?>
        ...
        <?php else: ?>
            <p>No grades found. You can create some using the form above. Default grades might have been seeded during setup.</p>
        <?php endif; ?>
    </div>
</body>
</html>
