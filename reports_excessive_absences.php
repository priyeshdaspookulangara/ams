...
$default_start_date = date('Y-m-d', strtotime('-29 days', strtotime($default_end_date))); // 30 days inclusive
...
$absence_threshold = isset($_GET['threshold']) ? (int)$_GET['threshold'] : 5; // Default threshold
...
} else { // Teacher - can see reports for sections they have attendance rights for
...
if (isset($_GET['view_report']) || $output_format == 'pdf') {
    if (empty($start_date) || empty($end_date) || $absence_threshold <= 0) {
        $message = "Please select a valid date range and a positive absence threshold."; $message_type = 'error';
        ...
        $filter_details_header = "Period: " . date("M j, Y", strtotime($start_date)) . " to " . date("M j, Y", strtotime($end_date)) .
                                 " | Threshold: " . $absence_threshold . " absences";
        ...
        if ($current_role == 'admin') {
            ...
            if ($selected_class_section_id !== 'all' && $selected_class_section_id > 0) {
                ...
                // Get selected class section name for header
                ...
                $filter_details_header .= " | Class Section: " . htmlspecialchars($cs_name_for_header);
            } else {
                 $filter_details_header .= " | Class Section: All Sections";
            }
        } elseif ($current_role == 'teacher') {
            $accessible_section_ids = get_teacher_attendance_accessible_sections($current_user_id, $conn); // Using attendance access for this report
            if (!empty($accessible_section_ids)) {
                if ($selected_class_section_id !== 'all' && $selected_class_section_id > 0) {
                    ...
                    $permission_to_proceed = true;
                    $filter_details_header .= " | Class Section: Your Accessible Sections";
                }
            } else {
                $message = "You do not have access to any class sections."; $message_type = 'error';
            }
        }

        if($permission_to_proceed && empty($message_type == 'error')){ // Proceed only if no permission error yet
            $report_sql .= " GROUP BY s.id, s.name, s.roll_number, class_section_display
                             HAVING COUNT(ar.id) >= $absence_threshold
                             ORDER BY total_absences DESC, cs.academic_year DESC, g.grade_name, d.division_name, s.name";

            $report_res = mysqli_query($conn, $report_sql);
            if ($report_res) {
                while ($row = mysqli_fetch_assoc($report_res)) $report_data[] = $row;
                if(count($report_data) == 0 && empty($message)) { $message = "No students found meeting the excessive absence criteria."; $message_type = 'info';}
...
if ($output_format == 'pdf' && empty($message_type == 'error') && !empty($report_data) ) {
    $pdf_html_content = "<h1>Excessive Absences Report</h1>";
    $pdf_html_content .= "<h2>" . htmlspecialchars($filter_details_header) . "</h2>";
    $pdf_html_content .= "<style> body { font-family: Arial, sans-serif; font-size: 10pt; } table { width: 100%; border-collapse: collapse; margin-top: 10px; } th, td { border: 1px solid #ddd; padding: 4px; text-align: left; } th { background-color: #f2f2f2; font-weight: bold; } h1,h2 {text-align:center;} </style>";
    $pdf_html_content .= "<table><thead><tr><th>Roll No.</th><th>Student Name</th><th>Class Section</th><th>Total Absences</th></tr></thead><tbody>";
    foreach ($report_data as $student) {
        ...
        $pdf_html_content .= "<td style='text-align:center;'>" . $student['total_absences'] . "</td>";
        $pdf_html_content .= "</tr>";
    }
    $pdf_html_content .= "</tbody></table>";
    generate_and_stream_pdf($pdf_html_content, "excessive_absences_report", 'P'); // Portrait
}
...
<div class="container-fluid mt-3">
    <h1>Excessive Absences Report</h1>
    ...
    <div class="card p-3 mb-3 bg-light no-print">
        <form action="reports_excessive_absences.php" method="GET">
             <div class="row g-3 align-items-end">
                ...
                <div class="col-md-12 mt-3 text-center"> <!-- Span full width for buttons or adjust layout -->
                    <button type="submit" name="view_report" value="html" class="btn btn-primary">View Report</button>
                    <button type="submit" name="output" value="pdf" formaction="reports_excessive_absences.php?<?php echo http_build_query(array_merge($_GET, ['output'=>'pdf', 'view_report'=>'pdf']));?>" class="btn btn-secondary ms-2">Download PDF</button>
                    <button type="button" onclick="window.print();" class="btn btn-info ms-2">Print HTML</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <?php if ((isset($_GET['view_report']) || $output_format == 'pdf_preview_debug') && empty($message_type == 'error') ): ?>
                    ...
                    <tbody>
                        <?php foreach ($report_data as $student): ?>
                            <tr>
                                ...
                                <td style="text-align:center;"><?php echo $student['total_absences']; ?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <?php elseif(isset($_GET['view_report'])): ?>
            <p class="alert alert-info mt-3">No students found meeting the excessive absence criteria.</p>
        <?php endif; ?>
    <?php endif; ?>
</div>
