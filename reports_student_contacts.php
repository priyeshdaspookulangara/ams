...
if ($current_role == 'admin') {
    ...
    // Teachers can see contacts for students in sections they can manage students for
    $manageable_section_ids = get_teacher_student_manageable_sections($current_user_id, $conn);
    ...
    if ($current_role == 'admin') {
        ...
        if ($selected_class_section_id !== 'all' && $selected_class_section_id > 0) {
            ...
            $filter_details_header = ($cs_name_q && mysqli_num_rows($cs_name_q)>0) ? mysqli_fetch_assoc($cs_name_q)['name'] : "Selected Section";
             ...
             $filter_details_header = "All Sections";
        }
    } elseif ($current_role == 'teacher') {
        // $manageable_section_ids already fetched for dropdown population
        if (!empty($manageable_section_ids)) {
            if ($selected_class_section_id !== 'all' && $selected_class_section_id > 0) {
    ...
    if($permission_to_proceed && empty($message_type == 'error')){
        ...
        if ($report_res) {
            while ($student_row = mysqli_fetch_assoc($report_res)) {
                $pg_sql = "SELECT parent_name, phone_number, email, relationship
            ...
            if(empty($report_data) && empty($message)) { $message = "No students found for the selected criteria."; $message_type = 'info';}
...
if ($output_format == 'pdf' && empty($message_type == 'error') && !empty($report_data) ) {
    $pdf_html_content = "<h1>Student Contact Information Report</h1>";
    $pdf_html_content .= "<h2>For: " . htmlspecialchars($filter_details_header) . "</h2>";
    $pdf_html_content .= "<style> body { font-family: Arial, sans-serif; font-size: 9pt; } table { width: 100%; border-collapse: collapse; margin-top: 10px; } th, td { border: 1px solid #ccc; padding: 4px; text-align: left; vertical-align:top; } th { background-color: #f0f0f0; font-weight: bold; } .parent-block { margin-bottom: 5px; padding-left:10px;} .parent-block strong {display: inline-block; min-width: 80px;} h1,h2 {text-align:center;} </style>";
    $pdf_html_content .= "<table><thead><tr><th>Roll No.</th><th>Student Name</th><th>Class Section</th><th>Parent/Guardian Contacts</th></tr></thead><tbody>";
    foreach ($report_data as $student) {
        ...
        if (!empty($student['parents'])) {
            foreach($student['parents'] as $parent){
                $pdf_html_content .= "<div class='parent-block'>";
                $pdf_html_content .= "<strong>Name:</strong> " . htmlspecialchars($parent['parent_name']) . "<br>";
                $pdf_html_content .= "<strong>Rel:</strong> " . htmlspecialchars($parent['relationship']) . "<br>";
                $pdf_html_content .= "<strong>Phone:</strong> " . htmlspecialchars($parent['phone_number']) . "<br>";
                $pdf_html_content .= "<strong>Email:</strong> " . ($parent['email'] ? htmlspecialchars($parent['email']) : '-') . "<br>";
                $pdf_html_content .= "</div>";
            }
        } else { $pdf_html_content .= "No contacts listed."; }
        $pdf_html_content .= "</td></tr>";
    }
    $pdf_html_content .= "</tbody></table>";
    generate_and_stream_pdf($pdf_html_content, "student_contact_report", 'P'); // Portrait
}

// --- HTML Output Logic ---
$page_title = "Student Contact Information Report";
ob_start();
?>
<div class="container-fluid mt-3">
        ...
        <form action="reports_student_contacts.php" method="GET">
             <div class="row g-3 align-items-end">
                ...
                <div class="col-md-auto">
                 ...
                 <div class="col-md-auto">
                    <button type="submit" name="output" value="pdf" formaction="reports_student_contacts.php?<?php echo http_build_query(array_merge($_GET, ['output'=>'pdf', 'view_report'=>'pdf']));?>" class="btn btn-secondary">Download PDF</button>
                </div>
                <div class="col-md-auto">
    ...
    <?php if ((isset($_GET['view_report']) || $output_format == 'pdf_preview_debug') && empty($message_type == 'error') ): ?>
                                    ...
                                    <?php if (!empty($student['parents'])): ?>
                                        ...
                                        <?php foreach ($student['parents'] as $parent): ?>
                                            <li class="mb-2 border-bottom pb-1">
                                                ...
                                                <small>Ph: <?php echo htmlspecialchars($parent['phone_number']); ?></small><br>
                                                <small>Email: <?php echo $parent['email'] ? htmlspecialchars($parent['email']) : '<em>Not set</em>'; ?></small>
                                            </li>
                                        <?php endforeach; ?>
                                        </ul>
        ...
        <?php elseif(isset($_GET['view_report'])): ?>
            <p class="alert alert-info mt-3">No students found for the selected criteria.</p>
        <?php endif; ?>
    <?php endif; ?>
</div>
